...
describeWithFlags('ObjectDetection', NODE_ENVS, () => {
	beforeEach(() => {
		spyOn(tfconv, 'loadGraphModel').and.callFake(() => {
			const model = {
				executeAsync: (x: tf.Tensor) => [tf.ones([1, 1917, 90]), tf.ones([1, 1917, 1, 4])],
				dispose: () => true,
			};
			return model;
		});
	...
	it('should allow custom model url', async () => {
		...
		expect(tfconv.loadGraphModel).toHaveBeenCalledWith('./web_model/model.json');
	});

	it('should allow custom model url', async () => {
		await load({ modelUrl: 'https://test.org/model.json' });

		expect(tfconv.loadGraphModel).toHaveBeenCalledWith('https://test.org/model.json');
	});
});
